<template>
	<view u-page class="tc-page">
				...
				<view class="u-demo-block">
					...
					<view class="u-demo-block__content">
						<u-row customStyle="margin-bottom: 10rpx">
							...
							<u-col span="6">
								<view class="demo-layout "><u--text :text="item.updateTime"></u--text></view>
							</u-col>
						</u-row>
						<u-row customStyle="margin-bottom: 10rpx">
							<u-col span="6">
		...
		<u-loading-page :image="loadingPageData.image" :loadingText="loadingPageData.loadingText" :icon-size="loadingPageData.iconSize" :font-size="loadingPageData.fontSize" :bg-color="loadingPageData.bgColor" :color="loadingPageData.color" :loading-color="loadingPageData.loadingColor" :loading ="this.status==='loading'" ></u-loading-page>
	</view>
</template>

<script>
import { getDetailLog } from '@/api/detaillog.js';
import {getDailyTaskTypes} from '@/api/writelog.js'
import { DAILY_TASK_TYPE_LIST } from '@/common/constants.js';
export default {
	data() {
		return {
			// form: {
	...
	methods: {
		...
		async loadDetailLogPage() {
			if (this.pageData.countTotal === null || this.pageData.countTotal > this.pageData.pageNum * this.pageData.pageSize) {
				...
				await getDetailLog(this.pageData).then(res => {
					...
					if (this.pageData.pageNum === 1) {
						...
						for (let i in res.data.rows) {
							let taskTypeName = this.dailyTaskTypes[0].filter(item => res.data.rows[i].taskType === item.id);
