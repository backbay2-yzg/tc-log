<template>
	<view class="container">
		<u--form labelPosition="left" :model="form" ref="uForm" class="form">
			...
			<u-form-item label="工作量(小时)" prop="workTime" borderBottom labelWidth="100">
				<u-number-box v-if="isWorkTimeInput" :min="0" :max="24" v-model="form.workTime" @change="workTimeChange" step="0.5" decimal-length="2"></u-number-box>
				<u--input v-else placeholder="请输入工作量(小时)" border="surround" v-model="form.workTime"></u--input>
				<u-switch v-model="isWorkTimeInput" @change="swWorkTimeChange"></u-switch>
			</u-form-item>

			<u-form-item label="加班时间(小时)" prop="workTime" borderBottom labelWidth="100">
				<u-number-box v-if="isOverTimeInput" :min="0" :max="24" v-model="form.overTime" @change="overTimeChange" step="0.5" decimal-length="2"></u-number-box>
				<u--input v-else placeholder="请输入加班时间(小时)" border="surround" v-model="form.overTime"></u--input>
				<u-switch v-model="isOverTimeInput" @change="swOverTimeChange"></u-switch>
			</u-form-item>
			<u-form-item label="发送给" borderBottom labelWidth="70" @click="showReceive = true" ref="uTaskPicker">
				<u--input v-model="dailyReceiveDisplayName" border="none" disabled disabledColor="#ffffff" placeholder="请选择接收方"></u--input>
				<u-icon slot="right" name="arrow-right"></u-icon>
			</u-form-item>
		...
		<u-popup :show="showReceive" @close="showReceive = false" @open="openReceivePop" :round="10" closeable closeIconPos="top-right" closeOnClickOverlay>
			<view class="popupBox">
				<scroll-view scroll-y="true" style="height: 500upx;" scroll-with-animation="true">
...
export default {
	...
	methods: {
		...
		submit() {
			...
			submitDailyLog(this.form).then(res=>{
