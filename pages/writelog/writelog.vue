	...
	<view class="container">
		<u--form labelPosition="left" :model="form" ref="uForm" class="form">
			<u-form-item label="项目名称" prop="projectName" borderBottom labelWidth="70" @click="showProjectPicker = true"
				ref="uProjectPicker">
				<u--input v-model="projectName" border="none" disabled disabledColor="#ffffff" placeholder="请选择项目名称">
				</u--input>
			...
			<u-form-item label="职位类型" prop="taskName" borderBottom labelWidth="70" @click="showTaskPicker = true"
				ref="uTaskPicker">
				<u--input v-model="taskName" border="none" disabled disabledColor="#ffffff" placeholder="请选择职位类型">
				</u--input>
			...
			<u-form-item label="工作量(小时)" prop="workTime" borderBottom labelWidth="120" class='form-item'>
				<u-number-box v-if="isWorkTimeInput" :min="0" :max="24" v-model="form.workTime" @change="workTimeChange"
					step="0.1" decimal-length="1"></u-number-box>
			...
			<u-form-item label="发送给" borderBottom labelWidth="70" @click="showReceive = true" ref="uTaskPicker">
				...
				<u--textarea v-model="dailyReceiveDisplayName" placeholder="请选择接收方" disabled disabledColor="#ffffff"
					autoHeight confirmType="done"></u--textarea>
			</u-form-item>
			...
			<view class="popupBox">
				<scroll-view scroll-y="true" style="height: 500upx;" scroll-with-animation="true">
					<u-checkbox-group size="27" borderBottom iconSize="18" iconPlacement="right" placement="column"
						@change="cbChange">
						<u-checkbox :customStyle="{ marginBottom: '16px' }" v-for="(item, index) in dailyReceiveNames"
							:key="index" :label="item.name" :name="item.id" :checked="item.checked" shape="circle">
						</u-checkbox>
					</u-checkbox-group>
				</scroll-view>
			</view>
